...
void dump(struct part *inven, int num_part);
void restore(struct part *inven, int *num_part);
...
int main(void){
    ...
    while(1){
        printf("\n\nOperation Codes: Insert: 'i', Search 's', Update Quantity: 'u', Change Price: 'c', Print: 'p', Download to file: 'd', Restore from file: 'r', Quit: 'q'\nEnter operation code: ");
        ...
        case 'p':
            ...
            break;
        case 'd':
            dump(inventory, num_parts);
            break;
        case 'r':
            restore(inventory, &num_parts);
            break;
        case 'q':
            exit(0);
            break;
...
void dump(struct part *inven, int num_part){
    char output_file[MAX_NAME+1];
    FILE *fp_out;

    printf("Enter name of output file: ");
    read_line(output_file, MAX_NAME+1);

    while((fp_out = fopen(output_file, "wb")) == NULL){
        printf("The file name used cannot be opened. Please try another filename.\n");
        printf("Enter name of output file: ");
        read_line(output_file, MAX_NAME+1);
    }

    fwrite(inven, sizeof(struct part), num_part, fp_out);
...
void restore(struct part *inven, int *num_part){
    ...
    read_line(input_file, MAX_NAME+1);

    while((fp_in = fopen(input_file, "rb")) == NULL){
        printf("The file name used cannot be opened. Please try another filename.\n");
        printf("Enter name of input file: ");
        read_line(input_file, MAX_NAME+1);
    }

    *num_part = fread(inven, sizeof(struct part), MAX_PARTS, fp_in);
    printf("Inventory successfully imported from %s.\n", input_file);
